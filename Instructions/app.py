...
def precipitation():
    ...
    precipation_query = session.query(*all_precipation_data).filter(Measurement.date > last_date).order_by(Measurement.date).all()
    ...
    for date, precipation in precipation_query:
        precipation_dict = {}
        precipation_dict["date"] = date
        precipation_dict["prcp"] = prcp
        precipation_data.append(precipation_dict)
